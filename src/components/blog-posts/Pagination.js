    ...
    <div className="mx-auto ">
      <ul className="mx-auto flex pl-0 list-none rounded my-2">
        ...
        {pageArray.map((pageNum) => (
          <li
            key={`pageNum_${pageNum}`}
            className={`relative flex py-2 px-3 leading-tight bg-white border border-gray-300 ${
              currentPage === pageNum
                ? "bg-blue-50 text-blue-900"
                : "text-blue-700"
            } ${
              currentPage === 1 && pageNum === currentPage ? "rounded-l" : ""
            } ${
              pageNum === numPages && currentPage === numPages
                ? "rounded-r"
                : "border-r-0"
            } hover:bg-gray-200`}
          >
